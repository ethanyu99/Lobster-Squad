...
import {
  Trash2, RefreshCw, Edit2, ExternalLink, Star, Settings, Share2,
  XCircle, FolderOpen, MessageSquare, Terminal, Upload,
  MoreHorizontal, Copy, Check, Cloud, Clock,
} from 'lucide-react';
...
function CopyBtn({ text, label }: { text: string; label?: string }) {
  ...
  return (
    ...
    >
      {copied ? <><Check className="h-2.5 w-2.5 text-emerald-500" />{label ? 'Copied' : ''}</> : <><Copy className="h-2.5 w-2.5" />{label || ''}</>}
    </button>
  );
}
    ...
    <>
      ...
      >
        ...
        <div className="px-4 pt-3 pb-2 flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] uppercase tracking-widest font-bold ${st.bg} ${st.text}`}>
                {st.label}
              </span>
              {isSandbox && (
          ...
          <div className="flex items-center gap-0.5 shrink-0 -mt-0.5">
            ...
            {webUiUrl && (
              <a href={webUiUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center h-7 w-7 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" title="Open Web UI" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-4 pb-2">
          <div className="flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-md bg-muted/50 dark:bg-muted/20 border border-border/40 dark:border-border/20 font-mono text-[11px]">
            <span className="text-muted-foreground truncate select-all">{instance.endpoint || '—'}</span>
            {instance.endpoint && <CopyBtn text={instance.endpoint} />}
          </div>
        </div>
